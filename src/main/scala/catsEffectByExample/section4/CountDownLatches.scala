...
object CountDownLatches extends IOApp.Simple {
  ...
  def announcer(latch: CountDownLatch[IO]): IO[Unit] = for {
    ...
    _ <- latch.release // gun firing :::RUN
    _ <- IO("GO GO GO").debug
  } yield ()
  ...
  object FileServer {
    val fileChunksList = List(
      "I love Scala.",
      "Cats Effects seems quite fun.",
      " low level concurrency with pure FP "
    )
  ...
  def appendFileContents(fromPath: String, toPath: String): IO[Unit] = {
    val compositeResource = for {
      ...
      writer <- Resource.make(IO(new FileWriter(new File(toPath))))(writer =>
        IO(writer.close())
      )
  ...
  override def run: IO[Unit] = {
    // IO.unit
    // sprint()
    downloadFile("myScalafile.txt", "/Users/sabuj/Downloads/cats-effect-by-examples/src/main/resources")
  }
